...
fn test_apply_patch_cli_allows_empty_add_file() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let target_path = tmp.path().join("empty.txt");
    ...
    assert_eq!(fs::read(&target_path)?, b"");
...
fn test_apply_patch_cli_preserves_eof_without_trailing_newline() -> anyhow::Result<()> {
    ...
    assert_eq!(fs::read(&target_path)?, b"first line\nsecond line");
...
#[test]
fn test_apply_patch_cli_reports_no_file_changes_for_noop_update() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let target_path = tmp.path().join("noop.txt");
    fs::write(&target_path, "same\n")?;
    ...
    assert_eq!(fs::read_to_string(&target_path)?, "same\n");
