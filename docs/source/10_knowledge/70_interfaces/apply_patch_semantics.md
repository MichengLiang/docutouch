...
- 作为 narrow compatibility behavior，若 numbered `@@` header 后紧接的第一条 old-side line 与 header visible text 相同，则该重复首行可从 header 锁定的同一行开始解释，而不是只能从其后继续搜索；
...
- numbered `@@` header 的 repeated-first-old-line compatibility path 属于 runtime compatibility semantics，而不是新的 public canonical form；
